...
class PlaylistTracksRepositoryImpl(
    ...
    override fun listPlaylistWithTracksCount(): Flow<List<PlaylistTracksCount>> {
        return playlistTrackDao.getPlaylistsTracksCount().distinctUntilChanged()
            .map { playlistTracksDbList ->
    ...
    override suspend fun addTrackToPlaylist(track: Track, playlistId: Int): Boolean {
        trackDao.insertTrackIfNotExists(trackConverter.map(track, false))
        try {
            val result = playlistTrackDao.insertPlaylistTrack(
                PlaylistTrackCrossRef(0,playlistId, track.trackId)
            )
            return result > 0L
        } catch (_: Exception) {
            return false
    ...
    override suspend fun getPlaylistTracks(playlistId: Int): PlaylistTracks {
        ...
        val tracks = playlistTrackDao.getTracksFromPlaylist(playlistId)
        return playlistTracksConverter.map(playlist,tracks)
    }

    override suspend fun deleteTrackFromPlaylist(playlistId: Int, trackId: Int) {
        playlistTrackDao.deleteNonFavouriteTrackFromPlaylist(playlistId, trackId)
    }
}